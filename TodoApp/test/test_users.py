...
def test_return_user(test_user):
    ...
    assert response.json()['username']=="riso0822"
    assert response.json()['email'] == "riteshsodwani@zohomail.in"
    assert response.json()['first_name'] == "Ritesh"
    assert response.json()['last_name'] == "Sodwani"
...
def test_change_passowrd_invalid_current_password(test_user):
